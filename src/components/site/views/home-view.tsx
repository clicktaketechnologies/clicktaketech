    ...
    <>
            ...
            <div>
              <Reveal>
                <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3.5 py-1.5 text-xs font-medium text-emerald-300">
                  ...
                  AI-Native Software Engineering · Multi-Agent Systems
                </span>
              </Reveal>
              <Reveal delay={0.05}>
                <h1 className="mt-6 text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl lg:text-6xl xl:text-7xl">
                  Engineering{" "}
                  <span className="text-gradient-emerald">Tomorrow&apos;s</span>{" "}
                  Intelligence, <span className="text-gradient-amber">Today.</span>
                </h1>
              </Reveal>
              <Reveal delay={0.1}>
              ...
              <Reveal delay={0.15}>
                <div className="mt-8 flex flex-wrap items-center gap-3">
                  <button
                    ...
                    className="group inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 text-sm font-semibold text-emerald-950 transition-all hover:bg-emerald-400 hover:shadow-[0_0_28px_-4px] hover:shadow-emerald-500/60"
                  >
                    Start Your Project
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-card/40 px-5 py-3 text-sm font-semibold text-foreground transition-all hover:border-emerald-500/40 hover:bg-card"
                  >
                    View Case Studies
              ...
              <Reveal delay={0.2}>
                <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    <Globe2 className="h-3.5 w-3.5 text-emerald-400" /> AWS · GCP · Azure
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <Zap className="h-3.5 w-3.5 text-emerald-400" /> 99.9% SLA
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <ShieldCheck className="h-3.5 w-3.5 text-emerald-400" /> GDPR · CCPA
                  </span>
                </div>
              </Reveal>
            </div>
            ...
            <div className="mt-16 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border/50 bg-border/40 sm:grid-cols-4">
              {STATS.map((s) => (
                <div key={s.label} className="bg-card/50 p-5 text-center backdrop-blur sm:p-6">
                  <div className="text-2xl font-bold tracking-tight text-gradient-emerald sm:text-3xl">
                    {s.value}
                  </div>
      ...
      <Section id="capabilities">
        <SectionHeading
          ...
          title={
            <>
              Six practices. <span className="text-gradient-emerald">One delivery engine.</span>
            </>
          }
          ...
          {CAPABILITIES.map((cap, i) => (
            <Reveal key={cap.title} delay={i * 0.05}>
              <button
                ...
                className="group relative h-full w-full overflow-hidden rounded-2xl border border-border/50 bg-card/40 p-6 text-left transition-all hover:-translate-y-1 hover:border-emerald-500/40 hover:shadow-[0_20px_60px_-20px] hover:shadow-emerald-500/20"
              >
                <div className="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-emerald-500/0 blur-2xl transition-all duration-500 group-hover:bg-emerald-500/20" />
                ...
                <div className="mt-5 flex items-center gap-1 text-sm font-medium text-emerald-400 opacity-0 transition-opacity group-hover:opacity-100">
      ...
      <Section className="border-y border-border/40 bg-card/20">
        <SectionHeading
          ...
          title={
            <>
              From discovery to <span className="text-gradient-emerald">deployment.</span>
            </>
          }
            ...
            <Reveal key={step.num} delay={i * 0.06}>
              <div className="relative h-full rounded-2xl border border-border/50 bg-card/40 p-6">
                ...
                {i < PROCESS_STEPS.length - 1 && (
                  <ArrowRight className="absolute -right-3 top-1/2 hidden h-5 w-5 -translate-y-1/2 text-emerald-500/40 lg:block" />
                )}
              </div>
            </Reveal>
          ))}
      ...
      <Section>
        <SectionHeading
          ...
          title={
            <>
              Numbers that <span className="text-gradient-amber">compounded.</span>
            </>
          }
          ...
          {IMPACT_STATS.map((s, i) => (
            <Reveal key={s.label} delay={i * 0.06}>
              <div className="h-full rounded-2xl border border-border/50 bg-card/40 p-6 text-center ring-gradient">
                <div className="text-3xl font-bold tracking-tight text-gradient-emerald sm:text-4xl">
                  {s.value}
                </div>
        ...
        <Reveal delay={0.2}>
          <div className="mt-8 flex justify-center">
            <button
              onClick={() => onNavigate("work")}
              className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-card/40 px-5 py-2.5 text-sm font-semibold text-foreground transition-all hover:border-emerald-500/40 hover:bg-card"
            >
      ...
      <Section className="border-y border-border/40 bg-card/20">
        <SectionHeading
          ...
          title={
            <>
              What clients <span className="text-gradient-emerald">say.</span>
            </>
          }
          ...
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.name} delay={i * 0.06}>
              <figure className="relative h-full overflow-hidden rounded-2xl border border-border/50 bg-card/40 p-6 sm:p-7">
                <Quote className="absolute right-5 top-5 h-10 w-10 text-emerald-500/10" />
                ...
                <figcaption className="mt-5 flex items-center gap-3 border-t border-border/40 pt-4">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 text-sm font-bold text-emerald-950">
                    {t.initials}
                  </span>
                  <div>
                    <div className="text-sm font-semibold text-foreground">{t.name}</div>
                    <div className="text-xs text-muted-foreground">{t.role}</div>
                    <div className="text-xs text-emerald-400/70">{t.location}</div>
                  </div>
                </figcaption>
              </figure>
            </Reveal>
      ...
      <Section>
        <Reveal>
          <div className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-card/40 to-amber-500/5 p-8 text-center sm:p-12 lg:p-16">
            <div className="absolute -left-20 -top-20 h-64 w-64 rounded-full bg-emerald-500/20 blur-3xl" />
            <div className="absolute -bottom-20 -right-20 h-64 w-64 rounded-full bg-amber-500/15 blur-3xl" />
            <div className="relative">
              <h2 className="mx-auto max-w-2xl text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
                Ready to deploy your{" "}
                <span className="text-gradient-emerald">AI workforce?</span>
              </h2>
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                <button
                  onClick={() => onNavigate("contact")}
                  className="group inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-6 py-3 text-sm font-semibold text-emerald-950 transition-all hover:bg-emerald-400 hover:shadow-[0_0_28px_-4px] hover:shadow-emerald-500/60"
                >
                  Start Your Project
                  <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                </button>
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-border/60 bg-card/40 px-6 py-3 text-sm font-semibold text-foreground transition-all hover:border-emerald-500/40 hover:bg-card"
                >
                  Email us
                </a>
              </div>
...
function HeroTerminal() {
  const lines = [
    { prompt: "$", text: "clicktake deploy --env=production", color: "text-emerald-400" },
    ...
    { prompt: "✓", text: "p99 120ms · 10M req/day · 99.9% SLA", color: "text-amber-400" },
  ];
  return (
    <div className="relative">
      <div className="absolute -inset-4 -z-10 rounded-3xl bg-gradient-to-br from-emerald-500/20 to-amber-500/10 blur-2xl" />
        ...
        <div className="grid grid-cols-3 gap-px border-t border-border/40 bg-border/30 text-center">
          {[
            ...
            <div key={b.k} className="bg-card/60 px-3 py-3">
              <div className="text-base font-bold text-emerald-400">{b.v}</div>
